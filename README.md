...
### Слияние веток
Слияние веток (merdge на англ.) - добавление изменений, созданных в одной ветке в другую (как правило, главную ветку). Чтобы выполнить слияние, перейдите в ветку, в которую хотите внести изменения и введите команду:
``` bash
...
Перед пулл-реквестомм рекомендуется подтянуть основную ветку проекта через `git pull`, сделать с ней `git merge` вашей ветки разработки, решить все конфликты, и только потом отправлять pull-request. Таким образом, вы будете успевать за изменениями в проекте.
