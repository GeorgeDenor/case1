...
Для того чтобы Git начал отслеживать измения в проекте необходимо сделать папку с файлами проекта Git-репозиторием. Для этого перейдите в диреторию проекта и введите команду
``` bash
$ git init
```
Git создаст в этой директории скрытую подпапку .git. Это рабочая папка служебная папка Git, в которой он хранит информацию о об изменнеиях в вашем проекте, настройки конфигурации и т.д.  
...
Теперь при вызове `git status` он сообщит вам `nothing to commit, working tree clean`, что значит, что в проекте нет изменений со времен предыдущего коммита
