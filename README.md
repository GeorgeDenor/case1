...
```
можно посмотреть информацию о конкретном коммите. А именно, какие изменения были в нём произведены.
...
#### Откат коммита
##### Жёсткий откат
Если необходимо вернуть проект к предыдущей его версии, удалив последние версии, введите команду:
```bash
...
##### Мягкий откат
Используя ключ `--soft` вместо `--hard` можно удалить последние коммиты, не меняя текущее состояние файлов. Такая команда переведёт некоторые файлы (изменившиеся или добавленные с момента выполнения коммита, на который произошёл откат) в состояние `Staged`.
##### Смешанный откат
Ключ `--mixed` (на самом деле используется по умолчанию) не удалит изменения, произведённые в проекте с момента коммита, на который произошёл откат, но и не добавит все эти файлы в `staged`. Изменённые файлы будут помечены как `Untracked` или `Modified`. Таким образом, можно будет откатить некоторые файлы выборочно командой `git restore`.
### Просмотр изменений
...
GitHub - система управления репозиториями, платформа для хранения IT-проектов и совместной работы над ними с использованием Git. Принадлежит Microsoft, имеет аналоги. Один из них - GitLab. Это opensource проект, который еще и позволяет создавать удалённые Git-репозиории на серверах компаний.  
...
```
Флаг `-u` (укороченная версия флага `--set-upstream`) сможет связать текущую ветку локального репозитория (в команде она указан ка master) с аналогичной в указанном удалённом репозитории `origin` (если там нет веток, то она создаст ветку).
...
Для того, чтобы обновить вашу локальную версию проекта до актуальной, необходимо перейти в ветку, которую вы хотите обновить (о ветках дальше), как правило, это main или master - основные ветки проекта. Затем введите команду 
``` bash
$ git pull
```
Эту команду стоит вводить каждый раз открытии кода проекта.
...
Для `commit~1` есть специальное сокращение - `commit~`

### Нет веток после клонирования репозитория
После клонирования у вас в локальном репозитории через `git branch` будет создана только локальная ветка. однако коммиты с других веток тоже будут склонированы. Просто введите `git checkout <имя ветки с удалённого репозитория>`, и тогда git сам создаст соответсвующую локальную ветку и сопоставит ей коммиты. Посмотреть доступные ветки, склонированные с удалённого репозитория можно, если ввести команду `git branch -a`. То есть если добавить флаг вывода скрытой информации.

### Слияние веток
Слияние веток (**merge** англ.) - добавление изменений, созданных в одной ветке в другую (как правило, главную ветку). Чтобы выполнить слияние, перейдите в ветку, в которую хотите внести изменения (например, main) и введите команду:
``` bash
$ git merge <branch name>
```
где `<branch name>` - имя ветки, с которой вы хотите произвести слияние.
После слияние git выведет похожую информацию:
``` bash
Updating 7c23be6..9887e32 # информация о том, какие коммиты были объединены 
# здесь это коммиты с 7c23be6 по 9887e32
Fast-forward # тип слияния
 README.md | 4 +++- # количество изменений в строках
 1 file changed, 3 insertions(+), 1 deletion(-) #подробности об изменениях
```
Здесь `Fast-forward` - тип слияния, результатом которого выступает линейная история коммитов.

Если посмотреть командой `git status --oneline` из ветки, в которую внесли изменения, то можно будет увидеть, что теперь на верхний коммит указывают сразу обе ветки. То есть вторая ветка не удалилась после слияния.
``` bash
$ git log --oneline
9887e32 (HEAD -> master, feature/diff) Добавить git diff в README
```
*При слиянии веток может открыться редактор VIM, для выхода введите `:q!`*

### Удаление веток
После слияния можно удалить ветку-донора. Это выполняется командой:
``` bash
$ git branch -D <branch name>
```
Где `<branch name>` - название ветки для удаления. Флаг `D` значит **Delete** - удаление. Можно указать и флаг `d` dvtcnj `D`, но такая команда будет "слабее". Она удалит ветку только если она была полностью объединена с другой — то есть если две ветки стали (или изначально были) частью одной истории.
*Удаление ветки из локального репозитория не удалит ветку на GitHub*

### Конфликты
Иногда при слиянии возникают конфликты. Это происходит, если при параллельном развитии двух веток вносились изменения , противорещаящие друг другу (например, была записана разная информация в одну и тужу строчку в разных ветках). В таком случае git пишет что-то вроде этого:
``` bash
...
```
Если автоматического слияние не произошло, вам следует разрешить конфликт вручную, сравнив две версии кода в разных ветках и внеся необходимые изменения.

### Pull request
Pull request - часть функционала GiHub (GitLab и т.д.). Это запрос на "подтягивание" изменений из вашей ветки разработки в основную ветку. Алгоритм такой:
...
У пулл-реквеста может быть два исхода:
...
Пулл-реквест можно сделать либо по ссылке, которую выводит Git при добавлении вашей ветки в удалённый репозиторий (команда `git push -u origin <название ветки, которую добавляете>`). Либо  его можно сделать из через интерфейс GitHub (кнопка `Pull requests` под имененем удалённого репозиория -> зелёная кнопка `New pull request`, далее разберетесь при знании английского).

За пулл реквестом следует **code-review** от ваших коллег. Они сделают это из вкладки **Files changed**
 
Во вкладке **Pull requests** можно посмотреть комментарии и обсудить изменения в коде. Вы также можете в любой момент добавить дополнительные коммиты в ветку, они автоматом присоединятся к пулл реквесту.

Далее, для объединения главной ветки с вашим реквестом нужно наажать кнопку **Merge pull request**. Скорее всего, это сделают руководители проекта.

Перед пулл-реквестомм рекомендуется подтянуть основную ветку проекта через `git pull`, объединить её с вашей веткой с помощью `git merge main` (вместо main - имя главной ветки), решить все конфликты, и только потом отправлять pull-request. Таким образом, вы будете успевать за изменениями в проекте.

## Ещё полезные команды
Здесь будут добавляться еще команды, которые я не успел обозреть в предыдущем параграфе.

### Удаление текущего привязанного удалённого репозитория

``` bash
$ git remote rm origin # эта команда удалит текущий origin
```
Вместо origin может быть другое имя, которым вы назвали свой удалённый репозиторий. Теперь вы можете связать свой локальный репозиторий с другим.

### Push в определённую, уже существующую ветку удалённого репозитория
...
### fast-forward и git push
Для `git push` важно состояние fast-forward между удалённой веткой и локальной. При наличии такого состояния git push просто перематывает состояние удалённой ветки до состояния локальной, добавляя туда коммитов.

Если удалённая ветка разошлась с локальной (туда запушили с другого устройства), то они уже не будут находиться в ff-состоянии. Тогда `git push` выведет похожую ошибку:
``` bash
# текущая ветка — main
$ git push
...
```
Далее описано несколько способов разрешения такого конфликта. Очевидно, что при командной паботек не стоит пушить всё в main, потому что тогда ветки у разных участников постоянно будут расходиться.
...
$ git push --force
```
Позволяет решить проблему n-ff состояния локальной и удалённой веток. Она грубо удалит коммиты, которые появились в удалённом репозитории после того коммита, с которым локальная ветка нахолдилась в состоянии ff. Довольно опасная команда, которая, тем не менее иногда позволяет починить "сломавшуюся" ветку.

#### Перебазирование ветки
Операция **rebase** позволяет изменить точку (коммит), от которой отделяется ветка. Так можно вернуть состояние ff между локальной и удалённой веткой. 
...
#### Пушить всё в main
Как упоминалось ранее, этот подход неудобен при работе в большой команде

#### Feature branch workflow
Работа  над любой новой функциональностью или исправлением ведётся в отдельной ветке. После изменения мёрджатся в основную ветку main. При это все коммиты в ветке main являются мёрдж-коммитами.

Такой подход позволяет разработчикам трудиться над новыми функциональностями независимо друг от друга и не затрагивать основную ветку кода в процессе.
...
##### Использование запросов на слияние при feature branch workflow
Мёрджи в main обычно производятся через запросы на слияние (пулл-ревесты или мёрж-реквесты).

На этапе слияния можно как запустить автоматические тесты, которые покажут, не сломают ли новые изменения логику проекта, или посмотреть изменения глазами (выполнить code review).

В review оцениваются следующие критерии:
* стиль кода;
* наличие/отсутствие очевидных проблем - ошибок, багов, уязвимостей;
* наличие лишних деталей в коде.
